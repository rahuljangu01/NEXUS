// client/src/pages/GroupsPage.jsx (FULL & CORRECTED CODE)
  ...
  return (
    <div className="p-6 lg:p-8 h-full flex flex-col">
      <header className="flex justify-between items-center mb-6">
        <div><h1 className="text-3xl font-bold text-white">Groups</h1><p className="text-gray-400 mt-1">Your spaces for collaboration.</p></div>
        <Button className="bg-indigo-600 hover:bg-indigo-500" onClick={() => setCreateDialogOpen(true)}>
      ...
      {myGroups.length > 0 ? (
        <>
          <div className="relative mb-4"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" /><Input placeholder="Search your groups..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 bg-slate-900/50 border-slate-700 focus:ring-indigo-500 rounded-full"/></div>
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 overflow-y-auto custom-scrollbar -mr-4 pr-4">
                ...
                {filteredGroups.map(group => {
                  ...
                  return (
                    <motion.div 
                        ...
                        animate="visible" 
                        exit={{ opacity: 0, y: -10 }} 
                        className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-800/50 cursor-pointer" 
                    ...
                    >
                      ...
                      <div className="flex-1 overflow-hidden">
                        <div className="flex justify-between items-center">
                           <h3 className={`font-semibold truncate ${unreadCount > 0 ? 'text-white' : 'text-gray-300'}`}>{group.name}</h3>
                        </div>
