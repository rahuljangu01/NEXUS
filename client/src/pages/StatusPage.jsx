// client/src/pages/StatusPage.jsx (FINAL, FULLY FEATURED, UNREDUCED CODE)
...
import "./Dashboard.css"; // For custom scrollbar

// Sub-component for the Upload Status dialog
...
const StatusPage = () => {
  ...
  // Logic for the full-screen status viewer timer and auto-progression
  useEffect(() => {
  ...
  // Render the full-screen status viewer if a group is active
  ...
  // Render main status list page
  return (
    <div className="p-6 lg:p-8 h-full overflow-y-auto custom-scrollbar">
        <header className="flex justify-between items-center mb-8"><h1 className="text-3xl font-bold text-white">Status</h1></header>
        {isLoading ? <div className="flex justify-center items-center h-full pt-20"><div className="animate-spin rounded-full h-16 w-16 border-b-2 border-indigo-500"></div></div> :
            <div className="space-y-8">
                <Dialog open={isUploadOpen} onOpenChange={setUploadOpen}>
                    <div className="flex items-center gap-4 p-2 rounded-lg hover:bg-slate-800/50">
                        <div className="relative cursor-pointer" onClick={() => { if(myStatusGroup) { setActiveStatusIndex(0); setActiveGroupIndex(statusGroups.findIndex(g => g.user._id === myStatusGroup.user._id)) } else { setUploadOpen(true) }}}>
                            <Avatar className="h-16 w-16"><AvatarImage src={currentUser.profilePhotoUrl}/><AvatarFallback>{currentUser.name.charAt(0)}</AvatarFallback></Avatar>
                            {!myStatusGroup && <div className="absolute -bottom-1 -right-1 bg-indigo-600 rounded-full p-1 border-2 border-[#0d1117]"><Plus className="h-4 w-4 text-white"/></div>}
                        </div>
                        <div className="flex-1" onClick={() => { if(myStatusGroup) { setActiveStatusIndex(0); setActiveGroupIndex(statusGroups.findIndex(g => g.user._id === myStatusGroup.user._id)) } else { setUploadOpen(true) }}}>
                            <p className="font-semibold text-white text-lg">My Status</p>
                            <p className="text-sm text-slate-400">{myStatusGroup ? `${myStatusGroup.statuses.length} updates` : "Add to your story"}</p>
                        </div>
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="text-slate-400 hover:bg-slate-700"><MoreVertical className="h-5 w-5"/></Button></DropdownMenuTrigger>
                ...
                <div>
                    <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Recent Updates</h2>
                        ...
                        {friendStatusGroups.map((group) => group.user && (
                            <motion.div key={group.user._id} className="cursor-pointer p-2 rounded-lg hover:bg-slate-800/50" onClick={() => { setActiveStatusIndex(0); setActiveGroupIndex(statusGroups.findIndex(g => g.user._id === group.user._id)); }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: statusGroups.findIndex(g => g.user._id === group.user._id) * 0.05 }}>
                                <div className="flex items-center gap-4">
                                    <div className="relative p-1 rounded-full bg-gradient-to-tr from-yellow-400 to-pink-500"><Avatar className="h-16 w-16 border-4 border-[#1e293b]"><AvatarImage src={group.user.profilePhotoUrl}/><AvatarFallback>{group.user.name.charAt(0)}</AvatarFallback></Avatar></div>
                                    <div><p className="font-semibold text-white">{group.user.name}</p><p className="text-sm text-slate-400">{group.statuses.length} new updates</p></div>
                                </div>
                            </motion.div>
                        ))}
