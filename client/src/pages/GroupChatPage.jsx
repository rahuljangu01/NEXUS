// client/src/pages/GroupChatPage.jsx (FULL CODE with ESLint Warning Fixed)
...
const GroupChatPage = () => {
    ...
    }, [groupId, currentUser, incomingCallData]);
    ...
    const leaveGroupCall = useCallback(async (isRejecting = false) => {
        ...
        peersRef.current = {};
        setStream(null);
        socketService.emit('leave-group-call', { groupId, userId: currentUser.id });
    }, [stream, group, groupId, groupCallState, fetchGroupData, currentUser.id]);
    ...
    useEffect(() => {
        ...
        const handleNewUserJoining = ({ from }) => {
            if (!stream || from.id === currentUser.id) return;
            const peer = new Peer({ initiator: true, trickle: false, stream });
            peer.on('signal', signal => {
                socketService.emit('send-signal-group', { signal, to: from.id, from: { id: currentUser.id, name: currentUser.name, profilePhotoUrl: currentUser.profilePhotoUrl } });
            });
    ...
    // <<< --- YAHI FIX HAI --- >>>
    // Humne .id, .name, .profilePhotoUrl ki jagah poora 'currentUser' object daal diya hai.
    }, [groupId, groupCallState, stream, currentUser]);
    ...
    const isAdmin = group.members.find(m => m.user._id === currentUser.id)?.role === 'admin';

    return (
        <div className="h-full w-full flex bg-[#0d1117] text-gray-300 overflow-hidden relative">
            <AnimatePresence>
                {groupCallState !== 'idle' && (
                    <GroupCallingUI 
                        group={group}
            ...
            <main className={`flex-1 flex flex-col relative ${groupCallState !== 'idle' ? 'blur-sm pointer-events-none' : ''}`}>
                <header className="flex items-center p-3 h-20 border-b border-slate-800 flex-shrink-0 bg-slate-900/70 backdrop-blur-lg z-20">
                    <Button variant="ghost" size="icon" className="h-10 w-10 text-gray-400 mr-2" onClick={() => navigate("/dashboard/groups")}><ArrowLeft className="h-5 w-5" /></Button>
                    <div className="flex items-center gap-4 cursor-pointer" onClick={() => setInfoPanelOpen(true)}>
                        <Avatar className="h-12 w-12"><AvatarImage src={group?.avatar} /><AvatarFallback>{group?.name?.charAt(0)}</AvatarFallback></Avatar>
                        <div>
                    ...
                    <div className="p-4 flex flex-col">
                        {messages.map((item, index) => {
                            ...
                            if (item._type === 'call') {
                                ...
                                const callerName = item.caller?._id === currentUser.id ? 'You' : item.caller?.name || 'Someone';
                                return (
                            ...
                            return (
                                <div key={item._id || index} className="w-full">
                                    ...
                                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex items-end gap-2 my-1 ${isSender ? "justify-end" : "justify-start"}`}>
                                        {!isSender && <Avatar className="h-8 w-8 self-end"><AvatarImage src={item.sender.profilePhotoUrl}/><AvatarFallback>{item.sender.name.charAt(0)}</AvatarFallback></Avatar>}
                                        <div className={`message-bubble max-w-md md:max-w-lg px-4 py-2 rounded-2xl ${isSender ? "bg-indigo-600 text-white sent" : "bg-[#2a2a36] text-gray-200 received"}`}>
                                            {!isSender && <p className="text-xs font-bold text-indigo-300 mb-1">{item.sender.name}</p>}
                                            <p className="text-sm break-words">{item.content}</p>
                                            <span className="text-xs opacity-70 float-right ml-2 mt-1">{format(new Date(item.createdAt), 'p')}</span>
                                        </div>
                                    </motion.div>
                                </div>
                            )
                ...
                <footer className="p-4 border-t border-slate-800 bg-slate-900/70 backdrop-blur-lg z-20 relative">
                    <AnimatePresence>
                        {showEmojiPicker && (
                            <motion.div 
                                ...
                                exit={{ opacity: 0, y: 10 }} 
                                className="absolute bottom-[85px] left-4 z-30"
                            >
                    ...
                    <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                        ...
                        <Button type="button" variant="ghost" size="icon" className="h-12 w-12 text-gray-400 hover:text-white flex-shrink-0" onClick={() => setShowEmojiPicker(!showEmojiPicker)}>
                        ...
                        <Input 
                            ...
                            className="flex-1 h-12 bg-slate-800 border-slate-700 rounded-full px-5 focus:ring-2 focus:ring-indigo-500" 
                            autoComplete="off"
                            onFocus={() => setShowEmojiPicker(false)}
                        />
                        <Button type="submit" size="icon" className="h-12 w-12 bg-indigo-600 hover:bg-indigo-500 rounded-full" disabled={!newMessage.trim() || isUploading}>
