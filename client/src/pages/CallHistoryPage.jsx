// client/src/pages/CallHistoryPage.jsx (FULL & CORRECTED CODE)
    ...
    <div className="p-6 lg:p-8 h-full flex flex-col">
      <header className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-400 mt-1">Your recent call history.</p>
        </div>
        {history.length > 0 && (
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar -mr-4 pr-4">
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <PhoneMissed className="h-20 w-20 text-slate-600" />
            <h2 className="mt-4 text-xl font-semibold text-white">No Call History</h2>
            <p className="text-slate-400">You haven't made or received any calls yet.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {history.map(call => {
              if (call.callType === 'group' && call.group) {
                ...
                return (
                  <div key={call._id} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-800/50">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-12 w-12"><AvatarImage src={call.group.avatar} /><AvatarFallback>{call.group.name.charAt(0)}</AvatarFallback></Avatar>
                      <div>
                        <h3 className="font-semibold text-white">{call.group.name}</h3>
                        <div className="flex items-center gap-2 text-sm text-slate-400">
                          <Users className="h-4 w-4 text-green-400" />
                          <span>Group Call started by {callerName}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="text-sm text-slate-500">{format(new Date(call.createdAt), 'p, MMM d')}</span>
                  ...
                  <div key={call._id} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-800/50">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-slate-400">
                          {isOutgoing ? <ArrowUpRight className="h-4 w-4 text-green-400" /> : <ArrowDownLeft className={`h-4 w-4 ${call.status === 'missed' ? 'text-red-400' : 'text-blue-400'}`} />}
